...
const processSoilMoistureData = (response: ThingSpeakResponse, period: string): { chartData: DataItem[]; xKey: string; dateRange: string } => {
  ...
  switch (period) {
    ...
    case 'weeks': {
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        response.feeds.forEach(entry => {
          const entryDate = convertToPhilippineTime(entry.created_at);
          if (entryDate >= start && entryDate <= end && entry.field3) {
            const value = parseFloat(entry.field3);
            if (!isNaN(value)) {
...
const ExportModal: React.FC<ExportModalProps> = ({
  ...
  const handleExportClick = async () => {
    ...
    try {
      ...
      console.log("Exporting soil moisture data:", exportData);
      
      // For demonstration, create a simple CSV export
      if (exportFormat === "CSV") {
        const csvContent = [
          [xKey.charAt(0).toUpperCase() + xKey.slice(1), "Soil Moisture (%)"],
        ...
        a.href = url;
        a.download = `soilmoisture-data-${currentOverview}.csv`;
        a.click();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg max-w-md w-96 p-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-semibold text-[#356B2C]">Export Soil Moisture Data</h3>
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#E6F0D3] border border-[#356B2C] rounded p-3 shadow-lg">
        ...
        <p className="text-[#356B2C]">{`Soil Moisture: ${payload[0].value}%`}</p>
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { chartData: newData, xKey: newXKey, dateRange: newDateRange } = processSoilMoistureData(response, overview);

...
const processTemperatureData = (response: ThingSpeakResponse, period: string): { chartData: DataItem[]; xKey: string; dateRange: string } => {
  ...
  switch (period) {
    ...
    case 'weeks': {
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        response.feeds.forEach(entry => {
          const entryDate = convertToPhilippineTime(entry.created_at);
          if (entryDate >= start && entryDate <= end && entry.field1) {
            const value = parseFloat(entry.field1);
            if (!isNaN(value)) {
...
const ExportModal: React.FC<ExportModalProps> = ({
  ...
  const handleExportClick = async () => {
    ...
    try {
      ...
      console.log("Exporting temperature data:", exportData);
      
      // For demonstration, create a simple CSV export
      if (exportFormat === "CSV") {
        const csvContent = [
          [xKey.charAt(0).toUpperCase() + xKey.slice(1), "Temperature (°C)"],
        ...
        a.href = url;
        a.download = `temperature-data-${currentOverview}.csv`;
        a.click();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg max-w-md w-96 p-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-semibold text-[#356B2C]">Export Temperature Data</h3>
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#E6F0D3] border border-[#356B2C] rounded p-3 shadow-lg">
        ...
        <p className="text-[#356B2C]">{`Temperature: ${payload[0].value}°C`}</p>
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { chartData: newData, xKey: newXKey, dateRange: newDateRange } = processTemperatureData(response, overview);

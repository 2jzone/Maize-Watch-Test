...
const processSoilPhLevelData = (response: ThingSpeakResponse, period: string): { chartData: DataItem[]; xKey: string; dateRange: string } => {
  ...
  switch (period) {
    ...
    case 'weeks': {
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        response.feeds.forEach(entry => {
          const entryDate = convertToPhilippineTime(entry.created_at);
          if (entryDate >= start && entryDate <= end && entry.field4) {
            const value = parseFloat(entry.field4);
            if (!isNaN(value)) {
  ...
  const handleExportClick = async () => {
    ...
    try {
      ...
      if (exportFormat === "CSV") {
        ...
        a.href = url;
        a.download = `soilphlevel-data-${currentOverview}.csv`;
        a.click();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg max-w-md w-96 p-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-semibold text-[#356B2C]">Export Soil pH Level Data</h3>
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#E6F0D3] border border-[#356B2C] rounded p-3 shadow-lg">
        ...
        <p className="text-[#356B2C]">{`Soil pH Level: ${payload[0].value}`}</p>
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { chartData: newData, xKey: newXKey, dateRange: newDateRange } = processSoilPhLevelData(response, overview);
        
        setChartData(newData);
        setXKey(newXKey);
        setDateRange(newDateRange);
      } catch (error) {
        console.error("Error fetching soil ph level data:", error);

...
const THINGSPEAK_READ_API_KEY =
  process.env.THINGSPEAK_READ_API_KEY || "EQ3MYH5XBDSB6K2A";
const THINGSPEAK_WRITE_API_KEY =
  process.env.THINGSPEAK_WRITE_API_KEY || "9T78QG1NCJMGHFH3";
...
const SensorData = iotConnection.model(
  "SensorData",
  sensorDataSchema,
  "sensor_readings"
);
...
class ThingSpeakService {
  constructor() {
    ...
    console.log(
      `ThingSpeak service initialized with channel ID: ${this.channelId}`
    );
  ...
  initializeAutoFetch() {
    console.log(
      `Setting up automatic ThingSpeak data fetch every ${FETCH_INTERVAL_SECONDS} seconds`
    );

    // Fetch immediately on startup
    this.syncLatestDataFromThingSpeak()
      .then((result) => {
        if (result) {
          console.log(
            `[${new Date().toISOString()}] Initial data fetch: Successfully saved new data to MongoDB`
          );
        } else {
          console.log(
            `[${new Date().toISOString()}] Initial data fetch: No new data to save`
          );
        }
      })
      .catch((error) => {
        console.error(
          `[${new Date().toISOString()}] Error in initial data fetch:`,
          error.message
        );
      });
    ...
    cron.schedule(`*/15 * * * * *`, async () => {
      try {
        console.log(
          `[${new Date().toISOString()}] Running scheduled data fetch...`
        );
        const result = await this.syncLatestDataFromThingSpeak();

        if (result) {
          console.log(
            `[${new Date().toISOString()}] Successfully saved new data to MongoDB`
          );
        } else {
        ...
        console.error(
          `[${new Date().toISOString()}] Error in scheduled data fetch:`,
          error.message
        );
      }
    });
  ...
  async fetchLatestDataFromThingSpeak() {
    try {
      ...
      console.error(
        "Error fetching latest data from ThingSpeak:",
        error.message
      );
      throw new Error(
        `Failed to fetch latest data from ThingSpeak: ${error.message}`
      );
    }
  }
  ...
  async syncLatestDataFromThingSpeak() {
    try {
      ...
      const manilaTimestamp = new Date(
        feedTimestamp.getTime() + 8 * 60 * 60 * 1000
      );
      ...
      if (existingData) {
        console.log(
          "Latest data already exists in MongoDB for timestamp:",
          manilaTimestamp
        );
        return false;
      }
      ...
      console.error(
        "Error syncing latest data from ThingSpeak:",
        error.message
      );
      throw new Error(
        `Failed to sync latest data from ThingSpeak: ${error.message}`
      );
    }
  }
  ...
  async syncDataFromThingSpeak() {
    try {
      ...
      console.log(
        `Received ${thingSpeakData.feeds.length} records from ThingSpeak`
      );
      ...
      console.log(
        `ThingSpeak sync complete. Saved ${savedCount} new data points.`
      );
      return savedCount;
    } catch (error) {
  ...
  async getHistoricalData(minutes = 60) {
    try {
      ...
      if (!mongoData || mongoData.length === 0) {
        console.log(
          `No historical data found for the past ${validMinutes} minutes`
        );
        return [];
      }
  ...
  async fetchFieldData(fieldNumber, results = 10) {
    try {
      if (isNaN(fieldNumber) || fieldNumber < 1 || fieldNumber > 8) {
        throw new Error(
          `Invalid field number: ${fieldNumber}. Must be between 1 and 8.`
        );
      }
      ...
      const response = await axios.get(
        `${this.baseUrl}/channels/${this.channelId}/fields/${fieldNumber}.json`,
        {
          params: {
            api_key: this.readApiKey,
      ...
      if (!response.data || !response.data.feeds) {
        throw new Error(
          `Invalid data received from ThingSpeak for field ${fieldNumber}`
        );
      }
  ...
  async getThingSpeakFieldData(fieldNumber, results = 10) {
    try {
      ...
      console.error(
        `Error processing field ${fieldNumber} data:`,
        error.message
      );
      throw error;
    }
  ...
  /**
   * Gets weekly overview data with flexible date range options
   * @param {Date} startDate - Optional start date
   * @param {Date} endDate - Optional end date
   * @param {boolean} useCalendarWeek - Whether to align to calendar week boundaries (default: false)
   * @returns {Promise<Object>} Weekly data with daily averages and summary
   */
  async getWeeklyOverviewData(
    startDate = null,
    endDate = null,
    useCalendarWeek = false
  ) {
    try {
      ...
      if (startDate && endDate) {
        if (useCalendarWeek) {
          // Use calendar week boundaries (Sunday to Saturday)
          queryStartDate = this.getStartOfWeek(new Date(startDate));
          queryEndDate = this.getEndOfWeek(queryStartDate);
          console.log(
            `Using calendar week from ${queryStartDate.toISOString()} to ${queryEndDate.toISOString()}`
          );
        } else {
          // Use exact date range provided
          queryStartDate = new Date(startDate);
          queryStartDate.setHours(0, 0, 0, 0);
          queryEndDate = new Date(endDate);
          queryEndDate.setHours(23, 59, 59, 999);
          console.log(
            `Using exact date range from ${queryStartDate.toISOString()} to ${queryEndDate.toISOString()}`
          );
        }
      } else {
        // Get the last 7 days of data (more practical for ongoing monitoring)
        queryEndDate = new Date();
        queryStartDate = new Date();
        queryStartDate.setDate(queryStartDate.getDate() - 6); // Last 7 days including today
        queryStartDate.setHours(0, 0, 0, 0);
        queryEndDate.setHours(23, 59, 59, 999);

        console.log(
          `Using last 7 days from ${queryStartDate.toISOString()} to ${queryEndDate.toISOString()}`
        );
      }

      console.log(
        `Querying data from ${queryStartDate.toISOString()} to ${queryEndDate.toISOString()}`
      );
      ...
      // Create a map for all days in the range (not just calendar week)
      ...
      while (currentDate <= queryEndDate) {
        const dayKey = this.formatDateKey(currentDate);
      ...
      const processedDailyData = Array.from(dailyData.values())
        .sort((a, b) => a.dayIndex - b.dayIndex) // Sort by chronological order
        .map((day) => {
          ...
          const totals = day.data.reduce(
            (acc, reading) => {
              ...
              if (
                reading.measurements.soil_ph &&
                reading.measurements.soil_ph !== 0
              ) {
          ...
          return {
            ...
            measurements: {
              temperature: parseFloat(
                (totals.temperature / totals.count).toFixed(2)
              ),
              humidity: parseFloat((totals.humidity / totals.count).toFixed(2)),
              soil_moisture: parseFloat(
                (totals.soil_moisture / totals.count).toFixed(2)
              ),
              soil_ph:
                totals.soil_ph_count > 0
                  ? parseFloat(
                      (totals.soil_ph / totals.soil_ph_count).toFixed(2)
                    )
                  : null,
              light_intensity: parseFloat(
                (totals.light_intensity / totals.count).toFixed(2)
              ),
            },
          };
        });
      ...
      const periodStats = this.calculatePeriodStats(processedDailyData);

      console.log(
        `Processed daily data: ${processedDailyData.length} days, ${data.length} total readings`
      );
      ...
      return {
        ...
        summary: {
          ...
          daysInRange: processedDailyData.length,
          daysWithData: processedDailyData.filter((day) => day.hasData).length,
        },
      };
    } catch (error) {
  ...
  /**
   * Gets data for the current calendar week (Sunday to Saturday)
   * @returns {Promise<Object>} Current calendar week data
   */
  async getCurrentCalendarWeekData() {
    const now = new Date();
    const startOfWeek = this.getStartOfWeek(now);
    const endOfWeek = this.getEndOfWeek(startOfWeek);

    return this.getWeeklyOverviewData(startOfWeek, endOfWeek, true);
  }

  /**
   * Gets data for the last N days (more practical for ongoing monitoring)
   * @param {number} days - Number of days to look back (default: 7)
   * @returns {Promise<Object>} Last N days data
   */
  async getLastNDaysData(days = 7) {
    const endDate = new Date();
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - (days - 1)); // Include today

    return this.getWeeklyOverviewData(startDate, endDate, false);
  }

  /**
   * Helper method to calculate period statistics (renamed from calculateWeeklyStats)
   */
  calculatePeriodStats(dailyData) {
    const dataWithValues = dailyData.filter((day) => day.hasData);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log(
    `Automatic data fetch scheduled every ${FETCH_INTERVAL_SECONDS} seconds`
  );
}
